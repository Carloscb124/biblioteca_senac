...
if ($uid > 0) {
  // KPIs (contando ITENS emprestados pelo usuário)
  $stmt = mysqli_prepare($conn, "
    SELECT
      ...
      SUM(CASE WHEN ei.devolvido=0 AND e.data_prevista IS NOT NULL AND e.data_prevista < CURDATE() THEN 1 ELSE 0 END) AS atrasados
    FROM emprestimos e
  ...
  // Histórico detalhado (1 linha por LIVRO emprestado)
  $stmt = mysqli_prepare($conn, "
    SELECT
      l.titulo,
      e.data_emprestimo,
...
$values = [(int)$kpi['abertos'], (int)$kpi['devolvidos'], (int)$kpi['atrasados']];
?>
          ...
          <table class="table table-base align-middle mb-0">
            ...
            <tbody>
              ...
              while ($h = mysqli_fetch_assoc($historico)) {
                ...
                $atrasado = ($devolvido === 0 && !empty($prev) && $prev < $hojeX);
              ?>
                <tr>
      ...
      <script>
        new Chart(document.getElementById('chartStatusUsuario'), {
          ...
          data: {
            ...
            datasets: [{ data: <?= json_encode($values) ?>, backgroundColor: ['#f39c12', '#27ae60', '#e74c3c'] }]
          },
